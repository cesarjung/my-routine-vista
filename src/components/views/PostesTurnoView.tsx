...
const META_POSTES = 5.0; // Meta: 5.0 postes por turno
...
export const PostesTurnoView = () => {
  ...
  const { data, isLoading, isError, refetch, isRefetching, lastUpdated, rawData } = usePostesTurnoData(selectedUnidadesIds);
  ...
  const [janela, setJanela] = useSessionState<number>('filter_janela_postesturno', 6);
  ...
  const { yMin, yMax } = useMemo(() => {
    ...
    let calculatedMin = Math.max(0, Math.floor((min - 0.5)));
    let calculatedMax = Math.ceil((max + 0.5));

    if (calculatedMin >= META_POSTES) calculatedMin = Math.floor(META_POSTES - 1);
    if (calculatedMax <= META_POSTES) calculatedMax = Math.ceil(META_POSTES + 2);
  ...
  const kpis = useMemo(() => {
    ...
    const withinTargetCount = validLatestVals.filter(v => v >= META_POSTES).length;
  ...
  // Dados para o Ranking (ordenado do maior para o menor)
  const rankingUnits = useMemo(() => {
    ...
    const maxVal = Math.max(...sorted.map(u => u._latestVal ?? 0), 7.0);
  ...
  const getHeatmapStyle = (val: number | null) => {
    ...
    if (val >= 5.5) {
      return { bg: '#E0F2FE', text: '#0369A1' }; // Azul (>= 5.5)
    }
    if (val >= 5.0) {
      return { bg: '#E7F6EC', text: '#15803D' }; // Verde (5.0 - 5.4)
    }
    if (val >= 4.0) {
      return { bg: '#FDF3DC', text: '#B45309' }; // Amarelo (4.0 - 4.9)
    }
    return { bg: '#FBE5E5', text: '#B91C1C' }; // Vermelho (< 4.0)
  };

  const getRankingBarColor = (val: number | null) => {
    if (val === null || val === undefined) return 'hsl(var(--muted))';
    if (val >= 5.5) return 'hsl(210, 85%, 45%)';
    if (val >= 5.0) return 'hsl(var(--success))';
    if (val >= 4.0) return 'hsl(var(--warning))';
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col h-full w-full items-center justify-center bg-background">
        ...
        <p className="text-sm font-medium text-muted-foreground">Carregando dados de Poste/Turno...</p>
      </div>
    );
  }
        ...
        >
          ...
          {activeHoverData.items?.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 text-xs mb-1 font-medium">
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: item.color || '#0369A1' }} />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider leading-none mb-1">
                PLANEJAMENTO · POSTE / TURNO
              </p>
              <h1 className="text-xl font-bold text-foreground leading-none">
                Média de postes instalados por turno
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Alternar cálculo entre contagem de postes > 0 ou turnos produtivos > 0"
              >
                <span
                  className={cn(
      ...
      <div style={{ zoom: zoomLevel } as React.CSSProperties} className="flex flex-col gap-6 p-4 pb-8 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
          ...
          <div className="bg-card border border-border rounded-xl p-4 shadow-[var(--shadow-card)] relative flex flex-col justify-between">
            ...
            <div className="my-1.5 flex items-baseline gap-1">
              ...
              <span className="text-xs text-muted-foreground font-medium">postes / turno</span>
            </div>
            <span className="text-[11px] text-muted-foreground">
              Meta {META_POSTES.toFixed(1).replace('.', ',')} · {kpis.latestMonth}
            </span>
          ...
          <div className="bg-card border border-border rounded-xl p-4 shadow-[var(--shadow-card)] relative flex flex-col justify-between">
            ...
            <span className="text-[11px] text-muted-foreground">
              ≥ {META_POSTES.toFixed(1).replace('.', ',')} no mês
            </span>
            <div className="w-3 h-3 rounded-[3px] bg-[hsl(var(--success))] absolute top-3.5 right-3.5" />
          </div>

          {/* KPI 3: Maior Produção */}
          <div className="bg-card border border-border rounded-xl p-4 shadow-[var(--shadow-card)] relative flex flex-col justify-between">
            <span className="text-[10px] font-bold uppercase tracking-[.08em] text-muted-foreground">MAIOR PRODUÇÃO</span>
            <div className="my-1.5 flex items-baseline gap-1">
              <span className="text-[30px] font-bold text-foreground tabular-nums leading-none">
                {kpis.bestVal > 0 ? kpis.bestVal.toFixed(2).replace('.', ',') : '-'}
              </span>
              <span className="text-xs text-muted-foreground font-medium">postes/turno</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full items-start">
          ...
          <div className="lg:col-span-8 border border-border rounded-xl bg-card p-4 shadow-[var(--shadow-card)] flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-foreground">Evolução de Postes por Turno</h2>
            ...
            {chartMode === 'grid' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
                {chartData.map(unit => {
                  ...
                  const isAboveMeta = latestVal !== null && latestVal >= META_POSTES;
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-sm text-foreground tabular-nums">
                            {latestVal !== null ? `${latestVal.toFixed(2).replace('.', ',')}` : '-'}
                          </span>
                          {variation !== 0 && (
                            <span 
                              className={cn(
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={miniData} margin={{ top: 6, right: 6, left: 6, bottom: 2 }}>
                            ...
                            <Tooltip 
                              ...
                              formatter={(val: any) => [val !== null && val !== undefined ? `${Number(val).toFixed(2).replace('.', ',')}` : '-', 'Postes/Turno']}
                              labelFormatter={(label: any) => `Mês: ${label}`}
                            />
                            <ReferenceArea y1={META_POSTES} y2={yMax} fill="hsl(var(--success))" fillOpacity={0.06} />
                            <ReferenceLine y={META_POSTES} strokeDasharray="4 4" stroke="hsl(var(--muted-foreground))" />
                            <Area
                              type="monotone"
                              dataKey="val"
                              stroke={
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={lineChartData} margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(v: any) => [v !== null && v !== undefined ? `${Number(v).toFixed(2).replace('.', ',')}` : '-', '']}
                    />
                    <ReferenceArea y1={META_POSTES} y2={yMax} fill="hsl(var(--success))" fillOpacity={0.05} />
                    <ReferenceLine 
                      y={META_POSTES} 
                      strokeDasharray="4 4" 
                      stroke="hsl(var(--muted-foreground))" 
                      label={{ value: 'Meta (5,0)', fill: 'hsl(var(--muted-foreground))', fontSize: 10, position: 'insideTopRight' }} 
                    />
                    {chartData.map(unit => {
                      const isSelected = unidadeAtiva === unit.name;
                      return (
        ...
        <div className="w-full border border-border rounded-xl bg-card shadow-[var(--shadow-card)] flex flex-col overflow-hidden">
          <div className="p-4 border-b border-border bg-muted/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-bold text-foreground">Detalhamento de Postes por Turno</h2>
              <p className="text-xs text-muted-foreground">Por mês: postes por turno na célula e, abaixo, a barra de produção do mesmo mês</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px] font-medium">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-muted-foreground">5,0 - 5,4</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded" style={{ backgroundColor: '#FDF3DC', border: '1px solid #B45309' }} />
                <span className="text-muted-foreground">4,0 - 4,9</span>
              </div>
              <div className="flex items-center gap-1.5">
                    ...
                    <tr
                      ...
                      onMouseEnter={() => {
                        setActiveHoverData({
                          ...
                          items: [
                            { label: 'Média Postes', value: `${row._mediaGeral.toFixed(2).replace('.', ',')}`, color: '#0369A1' }
                          ]
                        });
                      }}
                          ...
                          <td 
                            ...
                            onMouseEnter={(e) => {
                              ...
                              setActiveHoverData({
                                ...
                                items: [
                                  { label: 'Postes / Turno', value: val !== null && val !== undefined ? `${val.toFixed(2).replace('.', ',')}` : '-', color: '#0369A1' }
                                ]
                              });
                            }}
                          >
