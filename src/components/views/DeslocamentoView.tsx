...
export const DeslocamentoView = () => {
  ...
  const [zoomLevel, setZoomLevel] = useSessionState<number>('filter_zoom_deslocamento', 1);
  ...
  const { data, isLoading, isError, lastUpdated, refetch } = useDeslocamentoData(selectedUnidadesIds);
  ...
  const [janela, setJanela] = useSessionState<number>('filter_janela_deslocamento', 6);
  ...
  // Meses exibidos (Ordem cronológica crescente, filtrado pela Janela Temporal)
  ...
  const chartData = useMemo(() => {
    ...
    filteredData.forEach(row => {
      ...
      g.sumU += row.valDeslocamento;
      if (row.valDeslocamento > 0) g.countU += 1;
      ...
      gm.sumU += row.valDeslocamento;
      if (row.valDeslocamento > 0) gm.countU += 1;
    ...
    const resultadoFinal = Object.values(agrupado).map(u => {
      ...
      item._mediaGeral = denomGeral > 0 ? Number((u.sumU / denomGeral).toFixed(2)) : null;

      mesesExibidos.forEach(m => {
        if (u.meses[m]) {
          const denomMes = mediaTodosTurnos ? u.meses[m].countAM : u.meses[m].countU;
          item[m] = denomMes > 0 ? Number((u.meses[m].sumU / denomMes).toFixed(2)) : null;
  ...
  const { yMin, yMax } = useMemo(() => {
    ...
    let calculatedMin = Math.max(0, Math.floor((min - 0.1) / 0.2) * 0.2);
    let calculatedMax = Math.ceil((max + 0.1) / 0.2) * 0.2;

    if (calculatedMin >= META_HORAS) {
      calculatedMin = Math.floor((META_HORAS - 0.2) / 0.2) * 0.2;
    }
    if (calculatedMax <= META_HORAS) {
      calculatedMax = Math.ceil((META_HORAS + 0.2) / 0.2) * 0.2;
    }

    return {
      yMin: Number(calculatedMin.toFixed(1)),
      yMax: Number(calculatedMax.toFixed(1))
    };
  ...
  const kpis = useMemo(() => {
    ...
    const withinTargetCount = validLatestVals.filter(v => v <= META_HORAS).length;

    let worstVal = 0;
    let worstUnit = '-';
    chartData.forEach(u => {
      if (u._latestVal !== null && u._latestVal > worstVal) {
        worstVal = u._latestVal;
        worstUnit = u.name;
      }
    });
  ...
  // Dados para o Ranking (ordenado do menor para o maior deslocamento no mês corrente)
  const rankingUnits = useMemo(() => {
    const sorted = [...chartData].sort((a, b) => {
      const valA = a._latestVal ?? 99;
      const valB = b._latestVal ?? 99;
      return valA - valB;
    });

    const maxVal = Math.max(...sorted.map(u => u._latestVal ?? 0), 1.5);
  ...
  // Dados formatados para o LineChart (Modo B)
  ...
  const getHeatmapStyle = (val: number | null) => {
    ...
    if (val <= 0.85 * META_HORAS) {
      return { bg: '#E7F6EC', text: '#15803D' };
    }
    if (val <= META_HORAS) {
      return { bg: '#FDF3DC', text: '#B45309' };
    }
    if (val <= 1.25 * META_HORAS) {
      return { bg: '#FCEADB', text: '#C2410C' };
    }
  ...
  const getRankingBarColor = (val: number | null) => {
    ...
    if (val <= 0.85 * META_HORAS) return 'hsl(var(--success))';
    if (val <= META_HORAS) return 'hsl(var(--warning))';
    if (val <= 1.25 * META_HORAS) return 'hsl(var(--primary))';
  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col h-full w-full items-center justify-center bg-background">
        ...
        <p className="text-sm font-medium text-muted-foreground">Carregando dados de Deslocamento...</p>
      </div>
    );
  }
    ...
    >
        ...
        >
          ...
          {activeHoverData.items?.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 text-xs mb-1 font-medium">
              <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: item.color || '#15803D' }} />
      ...
      <div className="flex flex-col gap-2.5 p-4 shrink-0 border-b border-border sticky top-0 z-20 bg-background/85 backdrop-blur w-full">
        ...
        <div className="flex items-center justify-between gap-4 w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider leading-none mb-1">
                PLANEJAMENTO · DESLOCAMENTO
              </p>
              <h1 className="text-xl font-bold text-foreground leading-none">
                Média de tempo de deslocamento
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            {/* Switch Verdadeiro para Média todos os turnos */}
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground font-medium">Média todos os turnos</span>
              <button
                type="button"
                role="switch"
                aria-checked={mediaTodosTurnos}
                onClick={() => setMediaTodosTurnos(!mediaTodosTurnos)}
                className={cn(
                  "w-[26px] h-[15px] rounded-full transition-colors relative focus:outline-none",
                  mediaTodosTurnos ? "bg-primary" : "bg-muted-foreground/30"
                )}
                title="Alternar cálculo entre contagem de deslocamento > 0 ou turnos produtivos > 0"
              >
                <span
                  className={cn(
                    "w-[11px] h-[11px] rounded-full bg-white transition-transform absolute top-[2px] left-[2px]",
                    mediaTodosTurnos ? "translate-x-[11px]" : "translate-x-0"
                  )}
                />
              </button>
            </div>
        ...
        {/* Fileira 2: Janela Temporal e Filtros em Chips */}
      ...
      <div style={{ zoom: zoomLevel } as React.CSSProperties} className="flex flex-col gap-6 p-4 pb-8 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
          ...
          <div className="bg-card border border-border rounded-xl p-4 shadow-[var(--shadow-card)] relative flex flex-col justify-between">
            ...
            <div className="my-1.5 flex items-baseline gap-1">
              <span className="text-[30px] font-bold text-foreground tabular-nums leading-none">
                {kpis.avgLatest.toFixed(2).replace('.', ',')}
              </span>
              <span className="text-xs text-muted-foreground font-medium">h / turno</span>
            </div>
            <span className="text-[11px] text-muted-foreground">
              Meta 1,20h · {kpis.latestMonth}
            </span>
          ...
          {/* KPI 3: Menor Deslocamento */}
          <div className="bg-card border border-border rounded-xl p-4 shadow-[var(--shadow-card)] relative flex flex-col justify-between">
            <span className="text-[10px] font-bold uppercase tracking-[.08em] text-muted-foreground">MENOR DESLOCAMENTO</span>
            <div className="my-1.5 flex items-baseline gap-1">
              <span className="text-[30px] font-bold text-foreground tabular-nums leading-none">
                {kpis.worstVal < 99 ? `${kpis.worstVal.toFixed(2).replace('.', ',')}h` : '-'}
              </span>
            </div>
        ...
        {/* SEÇÃO 2 & 3: GRÁFICO PRINCIPAL (MODO A/B) + PAINEL DE RANKING */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 w-full items-start">
          
          {/* GRÁFICO PRINCIPAL (Col 8/12) */}
          <div className="lg:col-span-8 border border-border rounded-xl bg-card p-4 shadow-[var(--shadow-card)] flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-foreground">Evolução do Deslocamento</h2>
                <p className="text-xs text-muted-foreground">Visão temporal por unidade</p>
              </div>

              {/* Segmented Control dos Modos do Gráfico */}
            ...
            {/* MODO A: PAINEL POR UNIDADE (SMALL MULTIPLES) */}
            {chartMode === 'grid' && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
                {chartData.map(unit => {
                  ...
                  const isBelowMeta = latestVal !== null && latestVal <= META_HORAS;
                      ...
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-sm text-foreground tabular-nums">
                            {latestVal !== null ? `${latestVal.toFixed(2).replace('.', ',')}h` : '-'}
                          </span>
                          {variation !== 0 && (
                            <span 
                              className={cn(
                            ...
                            >
                              {variation > 0 ? `+${variation.toFixed(2).replace('.', ',')}` : variation.toFixed(2).replace('.', ',')}
                            </span>
                          )}
                        </div>
                      </div>

                      {/* Mini AreaChart com identificação visual dos meses */}
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={miniData} margin={{ top: 6, right: 6, left: 6, bottom: 2 }}>
                            ...
                            <Tooltip 
                              ...
                              formatter={(val: any) => [val !== null && val !== undefined ? `${Number(val).toFixed(2).replace('.', ',')}h` : '-', 'Deslocamento']}
                              labelFormatter={(label: any) => `Mês: ${label}`}
                            />
                            <ReferenceArea y1={yMin} y2={META_HORAS} fill="hsl(var(--success))" fillOpacity={0.06} />
                            <ReferenceLine y={META_HORAS} strokeDasharray="4 4" stroke="hsl(var(--muted-foreground))" />
                            <Area
                              type="monotone"
                              dataKey="val"
                              stroke={
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={lineChartData} margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
                    ...
                    <Tooltip 
                      ...
                      formatter={(v: any) => [v !== null && v !== undefined ? `${Number(v).toFixed(2).replace('.', ',')}h` : '-', '']}
                    />
                    <ReferenceArea y1={yMin} y2={META_HORAS} fill="hsl(var(--success))" fillOpacity={0.05} />
                    <ReferenceLine 
                      y={META_HORAS} 
                      strokeDasharray="4 4" 
                      stroke="hsl(var(--muted-foreground))" 
                      label={{ value: 'Meta (1,2h)', fill: 'hsl(var(--muted-foreground))', fontSize: 10, position: 'insideTopRight' }} 
                    />
                    {chartData.map(unit => {
                      const isSelected = unidadeAtiva === unit.name;
                      return (
          ...
          <div className="lg:col-span-4 border border-border rounded-xl bg-card p-4 shadow-[var(--shadow-card)] flex flex-col justify-between">
            ...
            <div className="pt-4 border-t border-border mt-4 text-[10px] text-muted-foreground">
              Barras proporcionais ao maior valor do mês. Menor é melhor.
            </div>
          </div>

        </div>

        {/* SEÇÃO 5: TABELA COM SPARKLINE E HEATMAP */}
        <div className="w-full border border-border rounded-xl bg-card shadow-[var(--shadow-card)] flex flex-col overflow-hidden">
          {/* Header da Tabela com Legenda de Faixas */}
          <div className="p-4 border-b border-border bg-muted/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-bold text-foreground">Detalhamento de Deslocamento</h2>
              <p className="text-xs text-muted-foreground">Por mês: horas médias por turno na célula e, abaixo, a barra de produção do mesmo mês</p>
            </div>

            {/* Legenda das 4 Faixas no Cabeçalho */}
            <div className="flex flex-wrap items-center gap-3 text-[11px] font-medium">
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded" style={{ backgroundColor: '#E7F6EC', border: '1px solid #15803D' }} />
                <span className="text-muted-foreground">≤ 1,02h</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded" style={{ backgroundColor: '#FDF3DC', border: '1px solid #B45309' }} />
                <span className="text-muted-foreground">≤ 1,20h</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-3 h-3 rounded" style={{ backgroundColor: '#FCEADB', border: '1px solid #C2410C' }} />
                <span className="text-muted-foreground">≤ 1,50h</span>
              </div>
              <div className="flex items-center gap-1.5">
              ...
              <tbody className="divide-y divide-border/60">
                {chartData.map((row) => {
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={() => {
                        setActiveHoverData({
                          ...
                          items: [
                            { label: 'Média Tempo', value: `${row._mediaGeral.toFixed(2).replace('.', ',')}h`, color: '#15803D' }
                          ]
                        });
                      }}
                    ...
                    >
                      {/* Unidade com Trilho Laranja se Selecionada */}
                      ...
                      {/* Coluna Tendência (Sparkline SVG) */}
                          ...
                          <td 
                            ...
                            onMouseEnter={(e) => {
                              ...
                              setActiveHoverData({
                                ...
                                items: [
                                  { label: 'Deslocamento', value: val !== null && val !== undefined ? `${val.toFixed(2).replace('.', ',')}h` : '-', color: '#15803D' }
                                ]
                              });
                            }}
                          >
                      ...
                      {/* Coluna Média Geral (Heatmap Pill) */}
                        ...
                        {(() => {
                          ...
                          return (
                            <div 
                              ...
                              style={{ backgroundColor: style.bg, color: style.text }}
                            >
                              {row._mediaGeral !== null ? row._mediaGeral.toFixed(2).replace('.', ',') : '-'}
                            </div>
                          );
                        })()}
                      </td>
