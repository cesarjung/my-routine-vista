...
subprocess.run(["git", "checkout", "HEAD", "src/components/ContentArea.tsx"], check=True)

file_path = "src/components/views/CarteiraDashboardView.tsx"
with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# Fix 1: Header scrollbar (remove no-scrollbar-custom, add pb-2)
...
# Fix 2: Prevent horizontal expansion by adding w-full and min-w-0 to the sticky header container
# The original might be: className="sticky top-0 z-[100] bg-background border-b border-border space-y-3 pt-4 px-6 pb-4"
content = content.replace(
...
# Fix 3: Card 2 (Postes Disponíveis)
content = content.replace(
...
# Fix 5: Card 10 (Capacidade Faturamento)
content = content.replace(
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("Restored successfully and applied fixes.")
