...
async function run() {
    // Query all routines to see the frequencies
  ...
  const req2 = await fetch(\`\${url}/rest/v1/tasks?select=id,title,due_date,routine_id&due_date=gte.2026-03-01T00:00:00-03:00&due_date=lte.2026-03-31T23:59:59-03:00&status=eq.pendente\`, {
    headers: { 'apikey': key, 'Authorization': \`Bearer \${key}\` }
  });
  const tasks = await req2.json();

  console.log('Tasks in March:', tasks.length);
  
  // Cross check tasks with monthly/weekly routines
  ...
  console.log('Tasks belonging to weekly routines this month:', weeklyTaskCount);
  console.log('Tasks belonging to monthly routines this month:', monthlyTaskCount);
}
run();
